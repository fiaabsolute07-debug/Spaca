/** Shared JSON envelope for the asset API routes: same-origin, session, §13.1 status codes, no URL logging. */
...
export async function assetRoute(request: Request, run: (actor: Actor | null) => Promise<unknown>, options: { anonymous?: boolean } = {}) {
  ...
  try {
    ...
    console.error('asset route failed', error instanceof Error ? error.name : 'unknown');
    return NextResponse.json({ error: 'The file request could not be completed' }, { status: 500 });
  }
}
