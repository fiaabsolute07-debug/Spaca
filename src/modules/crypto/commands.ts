...
const createCryptoPayment: CommandHandler = async ({ tx, actor, form }) => {
  ...
  // The checkout hold is a workload claim, or the entitlement of a DIGITAL purchase.
  const [hold] = await tx<Row[]>`select expires_at from app.workload_claims where order_id=${orderId} and state='HELD' and (expires_at is null or expires_at > now())
    union all select expires_at from app.digital_entitlements where order_id=${orderId} and state='HELD' and expires_at > now()`;
