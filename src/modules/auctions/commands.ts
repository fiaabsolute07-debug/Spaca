...
const createAuction: CommandHandler = async ({ tx, actor, form }) => {
  ...
  const [auction] = await tx<Row[]>`insert into app.auctions (service_id,seller_id,title,starting_price_minor,minimum_increment_minor,buy_now_price_minor,starts_at,ends_at,status)
    values (${service.id},${actor.id},${service.title},${starting.toString()},${increment.toString()},${buy?.toString() ?? null},${starts.toISOString()},${ends.toISOString()},'SCHEDULED') returning id`;
  await tx`insert into app.reservations (pool_id,auction_id,state,expires_at) values (${service.pool_id},${auction!.id},'HELD',${ends.toISOString()})`;
...
const buyNow: CommandHandler = async ({ tx, actor, form }) => {
  ...
  const [order] = await tx<Row[]>`insert into app.orders (buyer_id,creator_id,service_id,pool_id,source,source_ref,title,status,amount_minor,platform_fee_minor,currency,brief,terms,delivery_due_at)
    values (${actor.id},${auction.seller_id},${auction.service_id},${auction.pool_id},'AUCTION',${auction.id},${auction.title},'AWAITING_PAYMENT',${auction.buy_now_price_minor},0,'USD',
      'Auction purchase — confirm the final brief in the order workspace.',${JSON.stringify({ revision_limit: 1 })}::jsonb,now()+interval '72 hours') returning id`;
...
const closeAuction: CommandHandler = async ({ tx, actor, form }) => {
  ...
  if (!topBid) {
    ...
    if (auction.reservation_id) {
      await tx`update app.capacity_pools set reserved_units=greatest(reserved_units-1,0) where id=${auction.pool_id}`;
      await tx`update app.reservations set state='RELEASED' where id=${auction.reservation_id}`;
    }
    return { path: `/auctions/${auctionId}`, message: 'Auction closed with no valid bids; capacity released' };
  }
  const [order] = await tx<Row[]>`insert into app.orders (buyer_id,creator_id,service_id,pool_id,source,source_ref,title,status,amount_minor,platform_fee_minor,currency,brief,terms,delivery_due_at)
    values (${topBid.bidder_id},${auction.seller_id},${auction.service_id},${auction.pool_id},'AUCTION',${auction.id},${auction.title},'AWAITING_PAYMENT',${topBid.amount_minor},0,'USD',
      'Winning bid — confirm the final brief in the order workspace.',${JSON.stringify({ revision_limit: 1 })}::jsonb,now()+interval '24 hours') returning id`;
