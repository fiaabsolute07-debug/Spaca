...
/**
 * GET → 302 to a short-lived signed URL for an item auction picture, or its small card copy, that a visible listing
 * shows (open, sold or ended without bids; any state for its own seller). A copy is never served once its original is
 * quarantined. Anything else is 404.
 */
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const [asset] = await sql<{ bucket: string; object_key: string; filename: string; mime: string }[]>`select a.bucket,a.object_key,a.filename,a.mime
    from app.storage_assets a join app.item_listing_images i on (i.asset_id=a.id or i.thumb_asset_id=a.id)
      join app.storage_assets original on original.id=i.asset_id join app.item_listings l on l.id=i.listing_id
    where a.id=${id} and a.purpose='ITEM_IMAGE' and a.lifecycle_state='READY' and original.lifecycle_state='READY'
      and (l.status in ('OPEN','SOLD','NO_BIDS') or l.seller_id=${actor?.id ?? null}) limit 1`;
