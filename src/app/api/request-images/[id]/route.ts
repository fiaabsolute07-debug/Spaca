...
/**
 * GET → 302 to a short-lived signed URL for a campaign image that a visible campaign currently shows (the same
 * visibility as the campaign page: OPEN, FILLED or CLOSED, or any status for its own buyer). Anything else is 404.
 */
export async function GET(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const [asset] = await sql<{ bucket: string; object_key: string; filename: string; mime: string }[]>`select a.bucket,a.object_key,a.filename,a.mime
    from app.storage_assets a join app.request_images i on i.asset_id=a.id join app.requests r on r.id=i.request_id
    where a.id=${id} and a.purpose='REQUEST_IMAGE' and a.lifecycle_state='READY'
