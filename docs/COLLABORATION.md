...
| UX-WS | Workspace frame (persistent sidebar, back links, profile as its own area from the header avatar), color accents, Post a brief with category cards and chips, campaign images (migration 0025) and cards, order receipt, REQ-11 sort/filter/CSV, SEC-11 safe logs and secret scan, DSC-05 stale checkout browser test; tab strip built then removed at the user's request. Evidence `docs/evidence/claude-UX-WORKSPACE.md`, `docs/evidence/claude-SEC-11-DSC-05.md`, `docs/evidence/claude-AUDIT-2026-09-15.md` | ORD-01, REQ-11, DSC-05 PASS local; SEC-11 PARTIAL | GAPS | DONE |
| PERF | Performance campaigns (master §9.6): fixed fee plus capped view bonus, creator median frozen at hire, hold of fee + cap, one measurement per post with hold signals, settlement that splits release and refund. Mock metrics adapter. Migration 0026. Evidence `docs/evidence/claude-PERFORMANCE.md` | no §18 row; §9.6 product scope | UX-WS | DONE |
