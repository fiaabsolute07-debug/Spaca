...
| C6 | **Operator console UI.** `/admin` (queues overview), `/admin/disputes`, `/admin/cases`, `/admin/operations`, `/admin/moderation`, `/admin/users`, `/admin/flags`, `/admin/audit`, `/admin/orders/[orderId]`. Use only the W2-B read models and commands in `docs/UI_CONTRACT.md`; every form posts to `/api/commands` with a required reason field; non-operators get the 403/404 page. No private brief/delivery text in lists. Check at 360/768/1440 with the `moderator`/`finance`/`admin` dev personas. | `src/app/admin/**`, `src/components/admin/**` | SEC-12 (UI), OPS-05 (UI) | W2-B | TODO |
