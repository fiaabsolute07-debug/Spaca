...
- Fourth pass (user: these links belong in the account area in the corner of the screen, not shown as a sidebar): the sidebar is gone. `src/components/account-menu.tsx` puts the same groups (Workspace, Find work or Hire, Account › Profile) and Log out behind an "Account" button in the header's top-right corner, replacing the Workspace button and the separate Log out. Pages render at full width again; inner pages keep their back links, and the narrow-column CSS overrides were removed.
  - Fixed while verifying: the header's own `nav` styles had turned the menu's groups into a squeezed row of pill links (now scoped under `.header .account-menu-panel`). At 860px and below, the panel was right-aligned to a button that is not at the right edge and began 41px off-screen; on narrow screens it now spans the header's width.
  - Browser check after restarting the dev server: at 1069px the panel sits in the top-right corner (260px, one vertical list, Overview current, Log out at the bottom); at 375px it spans 20–355px with no horizontal overflow (My services current).
  - Reruns with the Account menu: `workspace-nav.spec.ts` (menu groups, current entry, Escape returns focus, outside click and navigation close it, back links, Profile) together with the auth-dialog and FND-04 tests: 7 passed in 2.0 minutes. An earlier run of the same specs, before the CSS scoping, also passed (8/8).
