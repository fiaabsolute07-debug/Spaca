...
- Workspace navigation: signed-in accounts get an "Account" button in the header's top-right corner. It opens a panel (`nav` "Account menu") with Workspace, Find work (creators) or Hire (buyers), Account › Profile, and Log out; the current page's entry has `aria-current="page"`. Escape (focus returns to the button), a click outside or following a link closes it. There is no sidebar and no name/email/photo block. Signed-out visitors see Log in / Get started instead.
