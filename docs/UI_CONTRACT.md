...
- **Account menu, account first (simplified 2026-09-17, modelled on Binance):** the header button shows the account's photo (or initial) before "Account", with a lime dot while setup is unfinished. The panel opens on a region "Signed in as": photo, name and one quiet line — **Creator** / **Buyer** (lime) · `@handle` (after setup). Then **Finish setup** (only while unfinished), then one list with icons and no group headings: creators Overview, My services, Orders, My applications; buyers Overview, Orders, My campaigns; a divider; **Wallet** with the newest linked wallet as `0x12ab…9f3c` or "Connect" (→ `/settings/profile#wallets`); Profile; then Log out. Links the header already offers (Explore, Campaigns and Post a brief, Open campaigns, Auctions, Fund) and new-item pages (New service, New auction) are not repeated.
- Phones: `.container` keeps the side gutter (`var(--gutter)`, 16px minimum) instead of running content to the screen edge.
- **Tab title:** "spaca | Creator campaigns marketplace for web3 launches | Hire crypto-native creators on X" (brand | what it is | what you get). Pages that set their own title use the template "Page | spaca".
- **Browser tab icon:** a lime `#D6F25E` circle with the dark `#121214` spaca bars, readable on light and dark tab strips (like a marketplace badge). `src/app/icon.svg` is the source; `src/app/favicon.ico` (16/32/48 px) and `src/app/apple-icon.png` (180 px, full-bleed lime) are generated from it with `tsx scripts/brand-icons.ts`.
...
## 2026-09-16 additions: work samples are the work

- **Adding a sample.** `/creator/services/new` opens its Work samples step with **Upload work samples** (`FileUploadField purpose="SAMPLE"`, up to three images, videos or PDFs, submitted as `sample_asset_ids`), then one optional pair — **Link to work online (optional)** and **What that work is** — for work that only lives on someone else's channel. Neither is required to save a draft; `publish_service` still refuses a service with no approved sample. An uploaded sample takes its title from the file name (extension and separators removed, 120 characters). `/creator/services` carries one **Work samples** panel above the service list — **Sample file**, **What this work is**, **Link to it online (needed when there is no file)**, a description and a **Show it on** picker of the creator's own live services (empty keeps it in the portfolio only) — and gives each service a **Work samples (n)** disclosure holding that service's gallery. The form is one per page, not one per service: a creator with hundreds of services would otherwise load hundreds of upload fields. A sample added there starts `PENDING` and says so.
- **Showing a sample.** `SampleGallery` (`src/components/samples/sample-gallery.tsx`) is the only way samples are shown: the public service page, the creator's public page, the Explore detail panel, the creator's own services page and the moderation queue. Each sample is a `figure`: an image fills a 4:3 frame, a video gets browser controls and plays in place, a PDF or a link-only sample shows a lime link (**Open the file ›** / **Open the post ›**); the caption carries the title, the description and, when an uploaded sample also has a link, **Where it ran ›**. The old `.sample-links` list and the single-link records are gone.
- **Serving a sample.** `GET /api/samples/<asset id>` → 302 to a short-lived signed URL. It answers for the creator who owns the file, for a moderator or admin, and for anyone once the sample is `PUBLIC` and `APPROVED`; everything else is 404, including a quarantined or unfinished file. Read models expose `storage_asset_id` only when the asset is `READY`, together with `asset_mime`, so a quarantined file cannot be addressed from a page.
- **Video.** The local signed-download route answers `Range` requests (206 with `content-range`, 416 past the end, `accept-ranges: bytes` always), because a video element asks for byte windows before it plays or seeks. `parseRange` lives in `src/modules/storage/range.ts`.
- **Moderation.** `/admin/moderation` shows the file above the decision, not only the sample id, so a picture is judged by looking at it.
- **Every file input now has a name.** `FileUploadField` labels itself with `<label htmlFor>` and a `useId` instead of a bare `<span>`, so `getByLabel('Product file')`, `getByLabel('Sample file')` and `getByLabel('Upload work samples')` all work and screen readers announce the field.

## 2026-09-17 additions: Connect X and the X profile in Explore (drizzle/0032)

- **Why a saved copy:** X bills every profile it returns (pay-per-use; $0.010 per user read when this was built). Buyers must see a creator's X photo and followers the moment they open them in Explore, so spaca reads a profile once when the creator connects and shows the saved copy everywhere; buyer views never read X.
- **Connect X** (creators only): Profile › "X account" › **Connect X** (`POST /api/x/connect`) → X's consent screen (OAuth 2.0 authorization code + PKCE S256, scopes `users.read tweet.read`) → `GET /api/x/callback`. The state is random, single-use, bound to the signed-in account, expires in 10 minutes and only its SHA-256 is stored (`app.x_oauth_states`); at most 10 attempts per 10 minutes. The callback reads `GET /2/users/me` once, revokes the token and stores no token. Refused: a buyer account, a state from another account, used or expired, a cancelled consent, a protected X account, an X account already connected to another spaca account, and a handle another creator listed as their own. Every outcome returns to the page with a notice; the callback answers `no-store` and `Referrer-Policy: no-referrer`.
- **Saved copy** (`app.x_profiles`, one per account, one account per X id): name, username, photo (X's 400 px image on `pbs.twimg.com` only), bio, location, verified and type, protected, followers, following, posts, listed, join date, `fetched_at`, `source` (`MOCK` or `X_API`). Connecting marks the matching linked X account `VERIFIED` with `verification_method='X_OAUTH'` (created if the creator had not listed it and has fewer than 10 links). **Disconnect X** (`disconnect_x`) deletes the copy and returns that link to self-reported; it stays because services may post on it.
- **Refresh:** a copy older than `X_REFRESH_AFTER_DAYS` (7) that someone views — a card opened in Explore (`POST /api/x/seen`, open to visitors), an Explore link with `selected=`, or the creator page — is queued once; the `refresh_x_profiles` job reads up to 100 queued copies in one `GET /2/users?ids=` with the app bearer token. Accounts X no longer returns are marked unavailable and keep their last details with a warning. The creator's own **Refresh from X** (`POST /api/x/refresh`) works at most once per 24 hours. All reads stop at `X_READS_MONTHLY_CAP` (1000 profiles) per month, counted in `app.x_api_usage`; connecting is then refused with a message and saved copies still show. Failed batches count `refresh_failures` (retried up to 5 times) and keep the error text.
- **Where buyers see it** (`XProfileCard`, region "On X"): Explore result cards carry a chip "𝕏 30.6K" ("30.6K followers on X"); the Explore detail panel and the creator page show the card: photo, name with a verified mark, `@username` linking to x.com (new tab, nofollow), Followers / Following / Posts in compact figures (followers in lime), bio, location and "Joined X Mar 2019", and a footer "Connected via X · updated 3 days ago". Profile › X account shows the same card with Refresh from X and Disconnect X. Linked accounts on the creator page read "Verified" for a connected X account.
- **Onboarding:** creators see an optional "Start from X" step above the setup form; after connecting, empty fields are filled from X (creator name, handle when it fits spaca's rules, bio of 40+ characters, `https://x.com/<username>` link) and stay editable.
- **Sandbox:** without `X_PROVIDER=live`, local development uses a stand-in for X: `/dev/x-authorize` ("Local sandbox · no real X account is used") lets the creator pick any username, whose profile is generated (stable per username, followers growing slightly each day; `gone_*` usernames act as accounts X no longer returns) with a generated photo from `/api/dev/x/avatar/<username>`. Sandbox copies are stored as `MOCK` and every card says **Sandbox X data**. The sandbox routes answer 404 unless the sandbox is on, and it is never on in production (`env:check` refuses `X_PROVIDER=mock` for staging and production).
...
- The field submits the wall clock under its own name and the reader's offset as `<name>_offset` — minutes east of UTC, taken for the moment picked, so a date across a daylight-saving change still lands right. Under the field it names the zone it used and what the chosen time is in UTC, the clock every date on the site is written in: `Asia/Bangkok (UTC+07:00) · Sep 27, 2026, 11:00 UTC`. The field's accessible name stays the label alone; the hint is `aria-describedby`.
- Without JavaScript no offset is sent, the wall clock is read as UTC exactly as before, and the hint reads **Read as UTC.** A prefilled value is rendered as its UTC wall clock and moved to the reader's clock once the page runs.
- `zoneOffset` refuses anything that is not a whole number of minutes within ±18 hours, so a forged form cannot move an instant to a place no zone could put it. `instantField(form, name)` reads the pair and is what `create_auction`, `create_request`, `update_request`, `request_deadline_extension` and the PUBLISH proof use. A `published_at` that already carries an offset or a `Z` is taken as written.
- `playwright.config.ts` pins `timezoneId: 'UTC'` so the browser and the test process share one clock; zone tests set their own with `test.use({ timezoneId })`.

## 2026-09-17 additions: web3 item auctions replace creator slot auctions (drizzle/0033)

- **Decision (user, 2026-09-17):** auctions sell web3 items — WL spots, GTD mints, pre-market tokens, points, airdrop or presale allocations, NFTs, or anything the seller describes — instead of creator service slots. Both projects (selling their own allocation) and holders (resale) can list. Money is escrow plus seller collateral. Creator slot auctions are gone from the product: `/creator/auctions/*` redirects to `/auctions/new`; `app.auctions` and `app.bids` stay as history and their commands and jobs are untouched.
- **`/auctions`** — heading "Web3 items, bid in the open.", **List an item**, a three-step strip (seller locks collateral · win and pay into escrow · confirm delivery), the sandbox line, filter chips by item type (from open listings, case-insensitive) and by seller (All sellers · Sold by the project · Resale), **Your auctions** (your listings and the auctions you bid on or won, with status, sale price or bid to beat, and end or "Closed"), **Live now** and **Starting soon** grids, and **Recently sold** (item, type, project, price, Buy now marker, date). With nothing live but something upcoming, Live now is omitted.
- **Card** — item type chip, origin badge (lime tint "Sold by the project" / outlined "Resale"), title, project · network · quantity, "Current bid" or "Starting at" in lime, "Ends in" / "Opens in" countdown, collateral, bid count, and Buy now while no bid exists.
- **`/auctions/new`** (signed in; finish setup first) — What you are selling: origin cards "I’m the project" (buyer accounts only, otherwise disabled) and "I’m reselling"; item type (free text with a datalist and suggestion chips); title; project; project link (optional, https added); network; quantity; description. Delivery: how the winner receives it; what the winner must give you; Deliver by. Auction: starting price; minimum increment (10); Buy now (optional, only until the first bid); your collateral with a live "At least $X, a fifth of the starting price. Covers N%"; Bidding opens (this minute) and Bidding closes (3 days), each a `TimeField` read in the person's zone. Submitted as JSON so a refusal keeps everything typed. Rules: collateral ≥ 1/5 of the starting price, Buy now above the starting price, start at most 5 minutes in the past, 5 minutes to 14 days long, delivery 1 hour to 365 days after the end, content policy screen.
- **`/auctions/<id>`** — chips (type, origin, state), title, project link · network · quantity; **About this item**; **Delivery** (method, then bidding opens and closes, what the winner provides, delivery deadline, 24 h to pay, 72 h to confirm); **Seller** (photo, name, account type, handle, their connected X card or "No X account connected"); **Bid history** ("You" or "Bidder N" by first bid, newest first). The sticky **Price and actions** panel: seller before opening — Lock $X (sandbox) or Cancel; while open — price, bids, countdown, bid field (minimum pre-filled and enforced by the field), Buy now, and "Seller collateral $X, N% of the current bid…"; after the sale, by role: winner pays with their wallet or other detail (only the seller sees it), the seller sees it and marks delivered with proof, the buyer confirms ("I received it") or opens a dispute, and claims payment plus collateral once the delivery deadline has passed; everyone else sees the price and state. Outcome lines say where the money went. Open listings refresh every 8 seconds.
- **Clocks** (job runner): close due listings (highest bid → sale with 24 h to pay; no bids → collateral back; collateral never locked → cancelled), expire unpaid wins (collateral back to the seller), confirm by silence after 72 h, and refund the buyer with the collateral when delivery is overdue.
- **Money (sandbox):** balanced ledger transactions between `sandbox_wallet:<user>`, `item_collateral:<listing>` and `item_escrow:<sale>`; both holding accounts return to zero at every outcome. No provider, no chain, no funds move; every money screen says so. The database refuses illegal sale and listing moves and freezes listing terms after the first bid.
- **`/admin/item-disputes`** (finance, support, admin read; finance or admin decide with a reason): buyer, seller, price, collateral, deadline, what the buyer gave, the seller's proof and the buyer's reason; outcomes RELEASE_TO_SELLER, REFUND_WITH_COLLATERAL (seller at fault) or REFUND (no fault, collateral back to the seller), audited.

## 2026-09-17 additions: item pictures, the gavel and the Beta tag (drizzle/0034)

- **Pictures:** a seller adds up to six pictures (the art, a project banner, an allowlist screenshot) in the listing form ("Pictures (optional)") or later on the listing page ("Pictures": Save / Replace / Remove all) while the listing waits for collateral or is open. The first is the cover. Uploads are the seller's own `ITEM_IMAGE` files (image only, 10 MB, bucket `public-items`), with the browser's small copy for cards; served by `/api/item-images/<id>` only while the listing is open, sold or ended without bids (any state for the seller), never after quarantine (which takes the picture and its copy together). A picture still shown cannot be deleted.
- **Cards** show the cover at 16:9; a listing without pictures shows a line drawing for its kind of item on the drawn grid — ticket (WL), ticket with shield (GTD), hexagon (mint), coin (token, pre-market), pie (presale, allocation), star (points), parachute (airdrop), frame (NFT), parcel (anything else). The **listing page** opens on the picture gallery (large picture, thumbnails to switch) or the same drawing, larger, with the item type.
- **Place bid is a gavel:** a lime button with a gavel over its sound block and "Place bid". Pressing it swings the gavel down onto the block (wind-up, strike with sparks, bounce, back; 0.52 s, still with reduced motion) while the bid is sent as JSON; the answer shows under the button and the price and minimum update in place. Without JavaScript it is a plain form post.
- **Beta:** the header's Auctions link carries a small lime-outlined "Beta" tag, and the page eyebrow reads "Auctions · Beta".


## 2026-09-17 additions: notifications move to a header bell; the overview is organised by question

- **Bell:** signed-in accounts get a bell before Fund and Account in the header. Its name is "Notifications", or "Notifications, N unread" with a lime count on the icon ("99+" from 100). Opening it loads the latest 12 in-app notifications (subject, body, "5 min ago"), unread ones marked with a dot; opening one follows its link and marks it read; "Mark all read" clears the count; "View all notifications ›" leads to `/notifications`. Escape returns focus to the bell; a click outside or a page change closes it. Links that are not a path on this site go to the overview.
- **`/notifications`** ("Notifications · What changed"): every in-app notification, newest first (latest 100), with Mark all read (a plain form post without JavaScript). Private: noindex, back link, sign-in prompt when signed out.
- **Overview (`/dashboard`)** no longer lists notifications. Top to bottom: the setup nudge (until onboarded); the heading with the two primary actions; **At a glance** — Needs your action, Active orders, Completed, Earned (creator) or Funded (buyer); **Needs your action** — up to 8 things this account must do now, soonest deadline first ("due in 2 d", "overdue by 3 h"), each linking to the page where it is done, disputes under review listed last; "You’re all caught up." when empty; **Shortcuts** — four links for the account type; **Recent orders** — the latest six with "View all ›".
- **What counts as an action:** creator — start work on a funded order with its brief, deliver the work, deliver the revision, answer a hire offer; buyer — pay for the order (and complete the brief), complete the brief, review the delivery, review applications to an open campaign; either account — pay into escrow or confirm receipt of a won item, deliver a sold item, lock collateral on a listing still waiting for it.
- **Narrow screens:** from 860 px down, the header puts the section menus on their own scrolling row under the logo and account buttons (was 640 px), so the bell, Fund and Account never push the page sideways.

## 2026-09-17 additions: the My services list stays a list
...
## 2026-09-17 additions: the new-service form asks the kind first

- The "What are you offering?" select and the three "If you chose PUBLISH / ACCESS / DIGITAL" blocks are one control: four options in a row (two by two on a phone) — **Create** "Content you make and deliver", **Publish** "A post on your own channel", **Access** "A live session with the buyer", **Digital** "Ready-made files to download" — each with its icon; the chosen one is lime.
- Under it, in the same box, only the chosen kind's terms: Create — a line saying there is nothing else to set; Publish — posting account ("Choose the account you post from"), post format, hours live, disclosure; Access — session length; Digital — license, what buyers get, copies, downloads, what buyers may do. The other kinds' fields stay in the form hidden, so a saved draft carries the same values as before.
- Then **The service** (title, niche, price, delivery time, scope) and **Work samples**. Same form on the page and in the Explore dialog; the dialog starts focus on the kind.

## 2026-09-17 additions: sign up with X only; email is added later (drizzle/0035)

- **Join dialog** ("Create your account"): choose Buyer or Creator, then **Continue with X** — the one filled button, disabled until a type is picked ("Choose Buyer or Creator to continue."). No email form. Under it: "Each account is one type and signs in with its own X account." and "You can add an email and password later in your account settings." In the sandbox: "Local sandbox: X opens a stand-in page. No real X account is used."
- **Sign-in dialog**: **Continue with X** first; "or"; **Continue with email** ("Email works once you have added one to your account.") opening the email and password form; then the local test accounts.
- After X: a new account lands on setup with "Signed up with X as @name." and its name, handle (creators), bio and link filled from X; an X account already on spaca signs in to it (from sign-up too: "@name already has a spaca account, so you are signed in to it."); an unknown X account on sign-in goes to the join dialog with "No spaca account signs in with @name yet. Choose Buyer or Creator to create one."; cancelling returns to the dialog with "X sign-in was cancelled." A creator needs a public X account.
- **Settings → Sign-in** (every account, above the X account panel): two rows — X ("@name", "sandbox X" when sandboxed; badge Signs in / Off) and Email and password (the address or "Not added"). Without an email: "Add an email (Gmail or any address) and a password to sign in without X…", fields Email address and Password (at least 12 characters), **Add email**; "Local sandbox: no email is sent, so the address is not verified." An email is added once and belongs to one account.
- **Disconnect X** is refused while X is the only way in ("X is how you sign in to this account. Add an email and password under Sign-in first, then disconnect X."), with a line under the button saying so; after an email is added, disconnecting also stops X from signing in.
