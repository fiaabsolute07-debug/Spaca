...
- Workspace frame: for a signed-in account, `SiteChrome` renders one persistent sidebar (`aside` "Workspace", `nav` "Workspace sections") beside workspace and marketplace pages (dashboard, buyer, creator, orders, requests, auctions, services, creators, explore). Navigation swaps the page next to it in the same tab; the current entry has `aria-current="page"`. Signed-out visitors and the landing, legal, auth, profile and admin pages have no sidebar. The profile is its own area: the sidebar has no identity block or Profile entry; the header shows the account photo (initial when there is none) as "Your profile", linking to `/settings/profile`, which renders outside the frame with "‹ Back to workspace" and the account type. Inside the frame, below 1240px wide, detail-page side panels stack under the main column, and Explore's filter column narrows to 220px (from 1024px up).
