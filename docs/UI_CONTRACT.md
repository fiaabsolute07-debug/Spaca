...
- Workspace frame: for a signed-in account, `SiteChrome` renders one persistent sidebar (`aside` "Workspace", `nav` "Workspace sections") beside workspace and marketplace pages (dashboard, buyer, creator, orders, settings, requests, auctions, services, creators, explore). Navigation swaps the page next to it in the same tab; the current entry has `aria-current="page"`. Signed-out visitors and the landing, legal, auth and admin pages have no sidebar. The sidebar has no identity block (name, email, photo), and the header has no profile button; the profile opens from the sidebar's Account › Profile entry inside the frame, and the profile page names the account type. Inside the frame, below 1240px wide, profile and detail-page side panels stack under the main column, and Explore's filter column narrows to 220px (from 1024px up).
