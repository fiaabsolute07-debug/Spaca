...
1. Open `/admin/orders/[orderId]` and `/admin/operations`; correlate `app.provider_operations` with `app.webhook_inbox`, `app.order_events`, `app.workload_claims` and `app.reconciliation_cases`. Check original reference, account, amount and currency; a screenshot/return URL is not funding evidence.
...
Verify: one funding effect in `app.ledger_transactions`/`app.ledger_entries`, one transition/outbox semantic effect, and one ACTIVE workload claim; otherwise keep the case open. Alerts for delayed funding and real-provider diagnosis tooling are NOT IMPLEMENTED.
