...
4. Reconcile on `/admin/operations` with `admin_retry_operation`, then verify provider-confirmed full REFUNDED or CANCELLED + PARTIALLY_REFUNDED. `release_ready_settlements` handles an agreed remainder. Cancellation or refund frees the creator's place once: the claim in `app.workload_claims` becomes RELEASED (W7-CAP).
