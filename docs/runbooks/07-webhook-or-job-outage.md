...
3. For local fixtures use `pnpm jobs:dev` against the original Next process, or one `POST /api/dev/jobs` as documented in the index. Read every job's examined/outcomes counts; HTTP 200 is not a per-operation success. The local inventory now includes verified `close_due_auctions` (W4-A AUC-05/06); Claude must integrate P4 changes before running the changing tree.
