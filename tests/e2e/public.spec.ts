...
test('anonymous visitors browse home, explore, service, requests and auctions without errors', async ({ page }) => {
  const errors: string[] = [];
  page.on('pageerror', error => errors.push(error.message));
  await visit(page, '/');
  await expect(page.getByRole('heading', { level: 1 })).toHaveText(/Your next big idea\.\s*The right creator\./);
  await visit(page, '/explore');
  await expect(page.getByRole('heading', { name: 'Find your kind of creative.' })).toBeVisible();
