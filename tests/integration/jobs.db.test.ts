...
describe.skipIf(!RUN_DB)('local jobs route', () => {
  it('runs every job once and refuses cross-origin browser calls', async () => {
    ...
    expect(body.reports.map((r) => r.job)).toEqual([
      'reprocess_webhook_inbox', 'chain_indexer', 'reconcile_provider_operations', 'expire_checkout_holds', 'expire_hire_offers', 'close_due_auctions', 'auto_accept_deliveries', 'release_ready_settlements', 'order_reminders', 'dispatch_notification_outbox', 'cleanup_storage',
    ]);
